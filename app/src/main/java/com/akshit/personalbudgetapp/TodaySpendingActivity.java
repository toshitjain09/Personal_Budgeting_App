    ...
    private void addItemSpentOn() {
        ...
        save.setOnClickListener(view -> {
            ...
            if (TextUtils.isEmpty(notes)){
                ...
                Data data = new Data(Item, date, id,itemNday,itemNweek,itemNmonth, notes, Integer.parseInt(Amount), months.getMonths(), weeks.getWeeks());
                expensesRef.child(id).setValue(data).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            Toast.makeText(TodaySpendingActivity.this, "Budget item added sucessfully", Toast.LENGTH_SHORT).show();
                        } else {
